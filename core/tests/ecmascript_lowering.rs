...
fn source_program(root: Value) -> SemanticProgram {
    serde_json::from_value(json!({
        ...
        "sources": [{
            "contract_version": "1.0.0",
            "source_id": "src:ecmascript-lowering",
            "specification_version": "1.0-draft.1",
            "frontend": {
                "id": "semantic_strling",
...
fn governed_ecmascript_profile() -> TargetProfile {
    serde_json::from_str(ECMASCRIPT).expect("governed ECMAScript 2024 profile must deserialize")
}

fn ecmascript_without_lookahead_evidence() -> TargetProfile {
    let mut value = serde_json::to_value(governed_ecmascript_profile()).expect("profile JSON");
    value["capabilities"]
        .as_array_mut()
        .expect("capability array")
        .retain(|capability| capability["capability_id"] != "assertions.lookahead");
    serde_json::from_value(value).expect("incomplete ECMAScript profile must deserialize")
}
...
#[test]
fn every_semantic_variant_lowers_to_explicit_ecmascript_structure() {
    ...
    let lowered =
        lower_ecmascript(&semantic, &target, &portability).expect("lowering must succeed");
    ...
    assert_eq!(lowered.case_matching, EcmascriptCaseMatching::Insensitive);
    assert_eq!(
    ...
    assert!(lowered
        .options
        .iter()
        .all(|option| option.option_id.as_str().starts_with("ecmascript.")));
    ...
    let EcmascriptOperation::Sequence(items) = &lowered.root.operation else {
        panic!("root must remain an ordered sequence");
    };
    assert_eq!(items.len(), 25);
    assert!(matches!(items[0].operation, EcmascriptOperation::Empty));
    assert!(matches!(
        items[1].operation,
        EcmascriptOperation::Alternation(_)
    ));
    assert_eq!(
    ...
    assert!(matches!(
        items[2].operation,
        EcmascriptOperation::Literal(ref value) if value == "é"
    ));
    assert!(matches!(
        items[3].operation,
    ...
    assert!(matches!(
        members[0],
        EcmascriptCharacterSetMember::Literal { value: 'é' }
    ));
    assert!(matches!(
        members[3],
    ...
    assert!(matches!(
        ...
        EcmascriptOperation::Repeat {
            min: 0,
            max: EcmascriptRepetitionMaximum::Unbounded,
            mode: EcmascriptRepetitionMode::Greedy,
            ..
        }
    ));
    assert!(matches!(
        ...
        EcmascriptOperation::Repeat {
            min: 1,
            max: EcmascriptRepetitionMaximum::Bounded(3),
            mode: EcmascriptRepetitionMode::Lazy,
            ..
        }
    ));
    let positions = [
    ...
    for (item, expected) in items[8..15].iter().zip(positions) {
        assert!(
            matches!(item.operation, EcmascriptOperation::Position(found) if found == expected)
        );
    }
    assert!(matches!(
        items[15].operation,
    ...
    assert!(matches!(
        items[17].operation,
        EcmascriptOperation::Backreference { slot: 1, .. }
    ));
    assert!(matches!(
        items[18].operation,
        EcmascriptOperation::Backreference { slot: 2, .. }
    ));
    let assertions = [
    ...
    for (item, expected) in items[19..25].iter().zip(assertions) {
        assert!(matches!(
            item.operation,
    ...
    let repeated = lower_ecmascript(&semantic, &target, &portability).expect("repeat lowering");
...
fn certified_atomic_literal_rewrite_is_applied_and_retained() {
    ...
    let lowered = lower_ecmascript(&semantic, &target, &portability).expect("rewrite lowering");
    ...
    assert!(matches!(
        ...
        EcmascriptOperation::Literal(ref value) if value == "a"
    ));
...
#[test]
fn required_compile_options_remain_typed_data() {
    let target = governed_ecmascript_profile();
    let semantic = program(literal("node:options.literal", "plain"));
    let portability = plan_for(&semantic, &target);

    let lowered = lower_ecmascript(&semantic, &target, &portability).expect("option lowering");

    assert_eq!(lowered.options.len(), 1);
    let unicode_mode = lowered
        .options
        .iter()
        .find(|option| option.option_id.as_str() == "ecmascript.unicode_mode")
        .expect("Unicode mode option");
    assert_eq!(unicode_mode.stage, OptionStage::Compile);
    assert_eq!(unicode_mode.selection, OptionSelection::Required);
    assert_eq!(
        unicode_mode.value,
        EngineOptionValue::String("u".to_owned())
    );
}
...
fn unresolved_and_unsupported_plans_fail_closed_with_stable_diagnostics() {
    ...
    let incomplete_target = ecmascript_without_lookahead_evidence();
    let unresolved_plan = plan_for(&unresolved_semantic, &incomplete_target);
    let unresolved = lower_ecmascript(&unresolved_semantic, &incomplete_target, &unresolved_plan)
    ...
    let unsupported = lower_ecmascript(&unsupported_semantic, &target, &unsupported_plan)
    ...
    assert_eq!(
        ...
        "STRL-ECMASCRIPT_LOWERING-0011"
    );

    let possessive_semantic = program(json!({
        "node_id": "node:unsupported.possessive",
        "kind": "repeat",
        "body": literal("node:unsupported.possessive.body", "a"),
        "min": 1,
        "max": null,
        "mode": "possessive"
    }));
    let possessive_plan = plan_for(&possessive_semantic, &target);
    let possessive = lower_ecmascript(&possessive_semantic, &target, &possessive_plan)
        .expect_err("uncertified possessive repetition must not lower");
    assert_eq!(
        possessive.code,
        EcmascriptLoweringErrorCode::UnsupportedRequirement
    );
    assert_eq!(
        possessive.diagnostics[0].code.as_str(),
        "STRL-ECMASCRIPT_LOWERING-0011"
    );
}

#[test]
fn stale_mismatched_and_non_ecmascript_inputs_are_rejected_before_lowering() {
    ...
    assert_eq!(
        lower_ecmascript(&second, &target, &plan)
    ...
    assert_eq!(
        lower_ecmascript(&plain, &changed_profile, &profile_plan)
    ...
    assert_eq!(
        lower_ecmascript(&plain, &pcre2, &pcre2_plan)
            .expect_err("non-ECMAScript target must fail")
            .code,
    ...
    assert_eq!(
        lower_ecmascript(&plain, &target, &wrong_version)
...
fn malformed_planner_and_rewrite_evidence_are_rejected() {
    ...
    assert_eq!(
        lower_ecmascript(&semantic, &target, &malformed_identity)
    ...
    assert_eq!(
        lower_ecmascript(&semantic, &target, &malformed_rewrite)
...
fn invalid_semantic_and_excessive_depth_fail_without_partial_output() {
    ...
    assert_eq!(
        lower_ecmascript(&invalid, &target, &plan)
    ...
    for depth in 0..MAX_ECMASCRIPT_LOWERING_DEPTH {
        root = Node::Atomic {
    ...
    deep.root = root;
    assert_eq!(
        lower_ecmascript(&deep, &target, &plan)
...
fn constructed_plan_validation_detects_capture_slot_corruption() {
    ...
    let target = governed_ecmascript_profile();
    let portability = plan_for(&semantic, &target);
    let mut lowered = lower_ecmascript(&semantic, &target, &portability).expect("lowering");
