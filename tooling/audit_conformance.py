...
It verifies that every fixture in tests/spec/ is tested by both Python
...
def run_python_conformance_tests(repo_root: Path) -> Set[str]:
    ...
    if result.returncode != 0 and "No module named pytest" in result.stderr:
        print(
            "ERROR: pytest is not installed. Please install Python dependencies:",
            file=sys.stderr,
        )
    ...
    if len(executed) == 0 and result.returncode == 0:
        print(
            "WARNING: Pytest ran but no conformance tests were found", file=sys.stderr
        )
        print(
            "Check that tests/unit/test_conformance.py exists and contains tests",
            file=sys.stderr,
        )
...
def run_java_conformance_tests(repo_root: Path) -> Set[str]:
    ...
    if test_count > 0:
        ...
        fixtures_dir = repo_root / "tests" / "spec"
        fixture_files = sorted(fixtures_dir.glob("*.json"))
        ...
        if test_count == len(fixture_files):
            ...
            print(
                f"Java test count ({test_count}) matches fixture count - assuming 100% coverage"
            )
        else:
            print(
                f"WARNING: Test count ({test_count}) doesn't match fixture count ({len(fixture_files)})"
            )
